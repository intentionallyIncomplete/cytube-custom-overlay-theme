...
```bash
npm run build          # produce dist/ + channel config template
# Serve dist/ (and related paths) from http://127.0.0.1:3000 — e.g. any static file server.
# Maintainer-local watch/serve helpers live in gitignored scripts/dev*.js (see scripts/README.md).
```

**CyTube wiring (local instance recommended — avoids HTTPS mixed content):**

1. Build BillTube: `npm run build`, then serve assets on `:3000` (or use a local `scripts/dev.js` if you maintain one)
2. Start local CyTube (see `sync/docker/README.md` in the CyTube repo)
3. Channel Settings → **Javascript** tab → paste a local channel snippet that points `BASE` at your server → Save JS
...
`dev/channel-settings.js` (when generated by maintainer helpers) is gitignored; release `channel_config_settings.js` is never mutated.
...
└── scripts/              # Shared tooling only — see scripts/README.md
    ├── build.js          # JS bundles + dist/billtube-fw.js; calls build-css.js
    ├── build-css.js      # src/styles/*.scss → dist/css/*.css (dart-sass)
    ├── build-options.js  # Shared esbuild options (imported by build.js)
    ├── verify-dist.js
    ...
    ├── purge-cdn.js
    ├── e2e-server.js     # Playwright fixture static server
    └── root-ownership.ts # Root ownership catalog (#207 / epic #206)
```
